...
const char *serial_file = "serial_log.txt";
const char *CAN_file = "CAN_log.CCLOG";
...
void log_can_to_sd(CAN_log_t *log_item, int folder) {
	static char can_file[80];
	sprintf(can_file, "/%d/%s", folder, CAN_file);
	FIL logfile;
	FRESULT res;
	FATFS fatfs_obj;
	UINT num_used;
	...
	res = f_open(&logfile, can_file, FA_WRITE | FA_OPEN_APPEND);
	...
	uint32_t can_id = (log_item->can_msg.header.IDE == CAN_ID_EXT) ? log_item->can_msg.header.ExtId : log_item->can_msg.header.StdId;
	for (int i = 0; i < 4; i++) {
...
void log_serial_to_sd(serial_log_t *log_item, int folder) {
	static char serial_file[80];
	sprintf(serial_file, "/%d/%s", folder, serial_file);
	FIL logfile;
	FRESULT res;
	FATFS fatfs_obj;
	UINT num_used;
	...
	// open log file to append to
	res = f_open(&logfile, serial_file, FA_WRITE | FA_OPEN_APPEND);
	...
	// write
	res = f_printf(&logfile, "%d\t%s\r\n", log_item->current_ticks, log_item->data);
