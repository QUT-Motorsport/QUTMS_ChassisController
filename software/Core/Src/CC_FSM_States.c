...
void state_idle_exit(fsm_t *fsm) {
	...
	CAN_TxHeaderTypeDef header = { .ExtId = readyToDrive.id, .IDE = CAN_ID_EXT, .RTR = CAN_RTR_DATA, .DLC = 1,
			.TransmitGlobalTime = DISABLE, };
	uint8_t data[1] = { 0xF };
	...
	CAN_TxHeaderTypeDef sirenHeader = { .ExtId = rtdSiren.id, .IDE = CAN_ID_EXT, .RTR = CAN_RTR_DATA, .DLC =
			sizeof(rtdSiren.data), .TransmitGlobalTime = DISABLE, };
...
void state_driving_enter(fsm_t *fsm) {
	...
	CAN_TxHeaderTypeDef runLeftHeader = { .StdId = runLeftScript.id, .IDE = CAN_ID_STD, .RTR = CAN_RTR_DATA, .DLC =
			sizeof(runLeftScript.data), .TransmitGlobalTime = DISABLE, };
	...
	CAN_TxHeaderTypeDef runRightHeader = { .StdId = runRightScript.id, .IDE = CAN_ID_STD, .RTR = CAN_RTR_DATA, .DLC =
			sizeof(runRightScript.data), .TransmitGlobalTime = DISABLE, };
...
*/
	...
	uint32_t brake_one_sum = 0;
	uint32_t brake_one_avg = 0;
	uint32_t brake_two_sum = 0;
	uint32_t brake_two_avg = 0;
	uint32_t accel_one_sum = 0;
	uint32_t accel_one_avg = 0;
	uint32_t accel_two_avg = 0;
	uint32_t accel_three_sum = 0;
	uint32_t accel_three_avg = 0;
	...
	if (osSemaphoreAcquire(CC_GlobalState->sem, SEM_ACQUIRE_TIMEOUT) == osOK) {
		...
		brake_travel[0] = map(brake_one_avg, CC_GlobalState->brakeMin[0], CC_GlobalState->brakeMax[0], 0,
		MAX_DUTY_CYCLE);
		brake_travel[1] = map(brake_two_avg, CC_GlobalState->brakeMin[1], CC_GlobalState->brakeMax[1], 0,
		MAX_DUTY_CYCLE);

		accel_travel[0] = map(accel_one_avg, CC_GlobalState->accelMin[0], CC_GlobalState->accelMax[0], 0,
		MAX_DUTY_CYCLE);
		accel_travel[1] = map(accel_two_avg, CC_GlobalState->accelMin[1], CC_GlobalState->accelMax[1], 0,
		MAX_DUTY_CYCLE);
		accel_travel[2] = map(accel_three_avg, CC_GlobalState->accelMin[2], CC_GlobalState->accelMax[2], 0,
		MAX_DUTY_CYCLE);
	 ...
	 */
	if (CC_GlobalState->faultDetected && !CC_GlobalState->ADC_Debug && CC_GlobalState->tractiveActive
	...
	if (((HAL_GetTick() - CC_GlobalState->inverter_cmd_ticks) >= INVERTER_CMD_TICK_COUNT)
...
//	}
	...
	if ((HAL_GetTick() - CC_GlobalState->readyToDriveTicks) % 27 == 0 && CC_GlobalState->rollingAccelValues[0] > 0
		...
		CAN_TxHeaderTypeDef brakeHeader = { .ExtId = brakeLightState.id, .IDE = CAN_ID_EXT, .RTR = CAN_RTR_DATA, .DLC =
				sizeof(brakeLightState.data), .TransmitGlobalTime = DISABLE, };
...
void state_debug_enter(fsm_t *fsm) {
	...
	uint8_t test[1] = { 0 };
	CAN_TxHeaderTypeDef header = { .ExtId = init.id, .IDE = CAN_ID_EXT, .RTR = CAN_RTR_DATA, .DLC = sizeof(test),
			.TransmitGlobalTime = DISABLE, };
