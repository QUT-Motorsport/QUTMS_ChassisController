...
void update_sensor_values() {
	...
	for (int i = 0; i < NUM_PEDAL_ACCEL; i++) {
		...
		if (pedal_value < current_sensor_values.pedal_accel_min[i]) {
			...
			pedal_value = current_sensor_values.pedal_accel_min[i];
			//}
		}

		if (pedal_value > current_sensor_values.pedal_accel_max[i]) {
			...
			pedal_value = current_sensor_values.pedal_accel_max[i];
			//}
		}
	...
	if (brake_val > current_sensor_values.brake_pressure_max) {
		brake_val = current_sensor_values.brake_pressure_max;
	}

	if (brake_val < current_sensor_values.brake_pressure_min) {
