...
USBD_StorageTypeDef USBD_Storage_Interface_fops_HS = { STORAGE_Init_HS,
		STORAGE_GetCapacity_HS, STORAGE_IsReady_HS, STORAGE_IsWriteProtected_HS,
		STORAGE_Read_HS, STORAGE_Write_HS, STORAGE_GetMaxLun_HS,
...
int8_t STORAGE_GetCapacity_HS(uint8_t lun, uint32_t *block_num,
	...
	if (HAL_SD_GetCardCSD(&hsd1, &pCSD) != HAL_OK) {
		return USBD_FAIL;
	}
...
int8_t STORAGE_IsReady_HS(uint8_t lun) {
	...
	HAL_SD_CardStatusTypeDef sd_status;
	if( HAL_SD_GetCardStatus(&hsd1, &sd_status) != HAL_OK) {
		return USBD_FAIL;
	}

	if (hsd1.State != HAL_SD_CARD_READY) {
		return USBD_FAIL;
	}
...
int8_t STORAGE_Read_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr,
		uint16_t blk_len) {
	/* USER CODE BEGIN 13 */
	if (HAL_SD_ReadBlocks(&hsd1, buf, blk_addr, blk_len, SD_TIME_OUT)
			!= HAL_OK) {
		return USBD_FAIL;
	}
...
int8_t STORAGE_Write_HS(uint8_t lun, uint8_t *buf, uint32_t blk_addr,
		uint16_t blk_len) {
	/* USER CODE BEGIN 14 */
	if (HAL_SD_WriteBlocks(&hsd1, buf, blk_addr, blk_len, SD_TIME_OUT)
			!= HAL_OK) {
		return USBD_FAIL;
	}
	return (USBD_OK);
